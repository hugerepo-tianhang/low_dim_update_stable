...
if __name__ == '__main__':
    ...
    proj_errors = projection(concat_matrix_diff, dx, dy, final_pca)
    ...
    for check_index in check_points:
        ...
        dx = pc1
        dy = pc2
        proj_errors = projection(concat_matrix_diff[check_index:], dx, dy, pca)

        distance_diff_to_centers = get_distances_diffs(final_pca, pca, concat_matrix_diff[check_index:])
        angle_diff_to_centers = get_angle_diffs(final_pca, pca, concat_matrix_diff[check_index:])
        dump_my(plot_dir_alg, proj_errors, f"all next proj_errors, check index: {check_index}")
        dump_my(plot_dir_alg, distance_diff_to_centers, f"distance_diff_to_centers, check index: {check_index}")
        dump_my(plot_dir_alg, angle_diff_to_centers, f"angle_diff_to_centers, check index: {check_index}")

        plot_2d_check_index(check_index, plot_dir_alg, distance_diff_to_centers,
                            "distance difference to check pca center vs final_pca center", "distance_diff", show=False)
        #TODO: use arccos instead of cos
        plot_2d_check_index(check_index, plot_dir_alg, angle_diff_to_centers, "cos ", file_name, show=False)

        plot_trajectory_wtih_error_only(check_index, plot_dir_alg, proj_errors, pca.explained_variance_ratio_[:2], show=args.show)
    ...
    for first_i in range(skip_num, concat_matrix_diff.shape[0]+1, skip_num):
        ...
        tic = time.time()
        new_PCA.fit(concat_matrix_diff[:first_i])
        toc = time.time()
        print(f"first_i {first_i} PCA took {toc - tic} seconds")

        num_to_explain = calculate_num_axis_to_explain(new_PCA, args.explain_ratio_threshold)
...
'''
    ...
    #
    #     logger.log(f"project all params")
    #     proj_xcoord, proj_ycoord, proj_errors = projection(threads_or_None, concat_matrix_diff, dx, dy, pca)
    ...
    #
    #
    # for first_i in range(1, total_num_points +1, skip_num ):
    #     #skip first args.skip_num_to_explain samples since those doesn't make sense
    #     print(f"num to explain first_i: {first_i}")
    #     if first_i <= 1:
    #         insofar_pca_explain_num.append(0)
    #         continue
    #     pca = PCA()
    #
    #     pca.fit(concat_matrix_diff[:first_i])
    #
    #     num_to_explain = calculate_num_axis_to_explain(pca, args.explain_ratio_threshold)
    #     insofar_pca_explain_num.extend([num_to_explain]*skip_num)
    #
    # short = len(proj_xcoord) - len(insofar_pca_explain_num)
    # insofar_pca_explain_num.extend([insofar_pca_explain_num[-1]] * short)
    '''
    PARALLEL PCA full intractable
    '''
    ...
    #
    #
    # this_run_dir_path = get_dir_path_for_this_run(args.machine, args.alg,
    #                                               args.total_timesteps, args.env_id,
    #                                               args.run_num)
    # folder = f"{this_run_dir_path}/joblib_memmap"
    #
    #
    ...
    #
    #
    # tasks = [first_i for first_i in range(1, total_num_points + 1, skip_num)]
    #
    # print(f"$$$$$$$$$$$$$TOTAL NUM OF POINTS: {total_num_points}")
    #
    # output_filename_memmap = os.path.join(folder, 'output_memmap')
    # insofar_pca_explain_num = np.memmap(output_filename_memmap, dtype=data.dtype,
    #                    shape=total_num_points, mode='w+')
    #
    # explain_ratio_threshold = args.explain_ratio_threshold
    #
    #
    # def parallel_pca_cell(concat_matrix_diff, first_i, insofar_pca_explain_num, skip_num):
    #     """Simulate a time consuming processing."""
    #     print(f"num to explain first_i: {first_i}")
    #     print("[Worker %d] for first_i %d" % (os.getpid(), first_i))
    #
    #     if first_i <= 1:
    #         insofar_pca_explain_num[first_i - 1] = 0
    #         return
    #
    #     pca = PCA()
    #
    #     pca.fit(concat_matrix_diff[:first_i])
    #
    #     num_to_explain = calculate_num_axis_to_explain(pca, explain_ratio_threshold)
    #     print("[Worker %d] num_to_explain for first_i %d is %f" % (os.getpid(), first_i, num_to_explain))
    #
    #     for i in range(skip_num):
    #         insofar_pca_explain_num[first_i - 1 - i] = num_to_explain
    #
    #
    # ###############################################################################
    # # Fork the worker processes to perform computation concurrently
    # tic = time.time()
    # Parallel(n_jobs=args.cores_to_use, max_nbytes=None, prefer=threads_or_None)(delayed(parallel_pca_cell)(data, first_i, insofar_pca_explain_num, skip_num)
    #                                                                             for first_i in tasks)
    # toc = time.time()
    # print('\nElapsed time computing the insofar PCA {:.2f} s\n'
    #       .format(toc - tic))
    #
    #
    # try:
    ...
    ############################################# milestone version using chunks of file size. to control chunk size
    ####################################go for file size
    ...
    # index = 0
    # while os.path.exists(get_full_param_traj_file_path(traj_params_dir_name, index)):
    #     logger.log(f"insofar params index: {index}")
    #
    #     concat_matrix_diff = get_concat_matrix_diff(traj_params_dir_name, index, final_concat_params)
    #     for first_i in range(skip_num, concat_matrix_diff.shape[0] + 1, skip_num):
    #         # skip first args.skip_num_to_explain samples since those doesn't make sense
