...
if __name__ == '__main__':
    ...
    if not os.path.exists(get_pcs_filename(intermediate_dir=intermediate_data_dir, n_comp=2))\
        ...
        for param in concat_matrix_diff:
            ...
            x, y = project_2D_final_param_origin(d=param, dx=the_9_and_10_pcs[0], dy=the_9_and_10_pcs[1])
    ...
    if not os.path.exists(get_eval_returns_filename(intermediate_dir=intermediate_data_dir,
        ...
        tic = time.time()
        thetas_to_eval = [final_concat_params + x * the_9_and_10_pcs[0] + y * the_9_and_10_pcs[1] for y in ycoordinates_to_eval for x in xcoordinates_to_eval]
