...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_id', default='Walker2d-v2', help='algorithm to train on')
    ...
    def load(dir_name, file_name):
        var_output_file = f"{dir_name}/{file_name}.csv"
        var_list = pd.read_csv(var_output_file, header=None).values[0]
        return var_list

    proj_xcoord = load(plot_dir_alg, "proj_xcoord")
    proj_ycoord = load(plot_dir_alg, "proj_ycoord")
    explained_variance_ratio = load(plot_dir_alg, "explained_variance_ratio")
    proj_errors = load(plot_dir_alg, "proj_errors")
    insofar_pca_explain_num = load(plot_dir_alg, "insofar_pca_explain_num")

    plot_contour_trajectory(plot_dir_alg, proj_xcoord, proj_ycoord, explained_variance_ratio, show=args.show)
    plot_trajectory_wtih_error_only(plot_dir_alg, proj_xcoord, proj_ycoord, proj_errors,
                                    explained_variance_ratio, show=args.show)
