...
import csv
import os
from stable_baselines.low_dim_analysis.eval_util import get_full_param_traj_file_path, get_full_params_dir, \
    get_dir_path_for_this_run, get_log_dir, get_save_dir
...
def do_cma(cma_args, first_n_pcs, orgin_param, save_dir, starting_coord, var):
    ...
    return mean_rets, min_rets, max_rets, np.array(optimization_path), np.array(
...
def main():
    ...
    cma_and_then_ppo_plot_dir = get_cma_and_then_ppo_plot_dir(plot_dir, pca_indexes,
    ...
    mean_rets, min_rets, max_rets, opt_path, opt_path_mean, best_pi_theta = do_cma(cma_args, pcs_to_use,
                                                                                   origin_param, save_dir,
    ...
    if len(pca_indexes) <= 2:
        ...
        xcoordinates_to_eval, ycoordinates_to_eval = gen_subspace_coords(cma_args,
                                                                         np.vstack((proj_coords, opt_path_mean_2d)).T)

        projected_after_ppo_params = do_proj_on_first_n(conti_ppo_params, pcs[pcs_to_plot_contour], origin_param)
        full_path = np.vstack((opt_path_mean_2d, projected_after_ppo_params))

        eval_returns = do_eval_returns(cma_args, intermediate_data_dir, pcs[pcs_to_plot_contour], origin_param,
                                       xcoordinates_to_eval, ycoordinates_to_eval, save_dir, pca_center=origin_name,
                                       reuse=False)
