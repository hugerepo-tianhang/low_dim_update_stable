...
from stable_baselines.a2c.utils import conv, linear, conv_to_fc, batch_to_seq, seq_to_batch, lstm, linear_with_mult
...
def mlp_extractor_with_mult_tensor(mult_tensors, flat_observations, net_arch, act_fun):
    """
    ...
    assert(len(mult_tensors) > 0)
    # Iterate through the shared layers and build the shared parts of the network
    for idx, layer in enumerate(net_arch):
        if isinstance(layer, int):  # Check that this is a shared layer
            layer_size = layer
            latent = act_fun(linear_with_mult(mult_tensors, latent, "shared_fc{}".format(idx), layer_size, init_scale=np.sqrt(2)))
        else:
    ...
    for idx, (pi_layer_size, vf_layer_size) in enumerate(zip_longest(policy_only_layers, value_only_layers)):
        if pi_layer_size is not None:
            ...
            latent_policy = act_fun(linear_with_mult(mult_tensors, latent_policy, "pi_fc{}".format(idx), pi_layer_size, init_scale=np.sqrt(2)))

        if vf_layer_size is not None:
            assert isinstance(vf_layer_size, int), "Error: net_arch[-1]['vf'] must only contain integers."
            latent_value = act_fun(linear_with_mult(mult_tensors, latent_value, "vf_fc{}".format(idx), vf_layer_size, init_scale=np.sqrt(2)))
...
def mlp_with_mult_tensor(mult_tensor, flat_observations, net_arch, act_fun):
    """
    Constructs an MLP that receives observations as an input and outputs a latent representation for the policy and
    a value network. The ``net_arch`` parameter allows to specify the amount and size of the hidden layers and how many
    of them are shared between the policy network and the value network. It is assumed to be a list with the following
...
class FeedForwardWithMultiPolicy(ActorCriticPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", num_comp=2, **kwargs):
        super(FeedForwardWithMultiPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse,
                                                 scale=(feature_extraction == "cnn"))
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            if feature_extraction == "cnn":
                ...
                pi_latent, vf_latent = mlp_extractor_with_mult_tensor(self.multi_tensors, tf.layers.flatten(self.processed_obs), net_arch, act_fun)

            self.value_fn = linear_with_mult(self.multi_tensors, vf_latent, 'vf', 1)

            self.proba_distribution, self.policy, self.q_value = \
                self.pdtype.proba_distribution_from_latent(pi_latent, vf_latent, init_scale=0.01, mult_tensors=self.multi_tensors)
...
class MlpMultPolicy(FeedForwardWithMultiPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(MlpMultPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
